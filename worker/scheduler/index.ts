...
export async function dispatch(event: Event, env: Env, source: RunSource = "CLOUDFLARE_CRON") {
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      throw new Error("AUTOMATION_REDIRECT_REJECTED");
    }
    stage = "JSON_RESPONSE";
    ...
    message = `Automation failed at ${stage}; HTTP ${httpStatus ?? "not received"}; next scheduled retry remains enabled`;
  }
  console.log(JSON.stringify({ timestamp: new Date().toISOString(), type, source, status, message, stage, httpStatus,
